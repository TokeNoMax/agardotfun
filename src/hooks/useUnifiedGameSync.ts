...
export const useUnifiedGameSync = ({
  ...
  const connect = useCallback(async () => {
    ...
    console.log('[Hook] Attempting to connect to unified game sync:', roomId);
    ...
    try {
      ...
      // Créer le nouveau service unifié
      ...
      if (connected) {
        ...
        toast({
          ...
          description: "Connexion temps réel unifiée active !",
          duration: 2000,
        });

        console.log('[Hook] Successfully connected to unified game sync');
        return true;
      } else {
        console.error('[Hook] Failed to connect to unified game sync');
        return false;
      }
    } catch (error) {
  ...
  // Fonction pour broadcaster la position (optimisée avec les deux fréquences)
